...
export function pdfDefinition(c, theme = 'light') {
  ...
  function imageBlock(image, fullPage = false) {
    if (!raster(image.data)) return text('Image unavailable.', { style: 'note' });
    const key = 'image' + (++imageId); images[key] = image.data;
    return { image: key, fit: [463, fullPage ? 630 : 460], alignment: 'center', margin: [0, 10, 0, 5], ...(fullPage ? { pageBreak: 'before' } : {}) };
  }
  function block(b) {
    if (b.type === 'asset') {
      ...
      if (a.pages) a.pages.forEach((page, i) => { nodes.push(imageBlock(page, i > 0), text(page.name, { style: 'caption' })); });
  ...
  content.push(text('PERSONAL EXPORTER  /  CONVERSATION', { fontSize: 8, characterSpacing: 1.4, color: colors.green, bold: true, margin: [0, 0, 0, 12] }));
  ...
  numberedMessages(c.messages).forEach(message => {
    ...
    function flush() {
      ...
      content.push({ table: { widths: ['*'], headerRows: 1,
        ...
        }, margin:[0,8,0,6], ...(resumeOnNewPage ? {pageBreak:'before'} : {}) });
      segment = []; continuation = true; resumeOnNewPage = false;
    }
  ...
  return {
    ...
    footer: (current, total) => ({ columns: [
      { text: 'PERSONAL EXPORTER', fontSize: 7, characterSpacing: 1, color: colors.muted },
