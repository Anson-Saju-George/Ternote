...
export async function advancedTests({ ui, evaluate, page, send, sleep, fixtureSessions, output }) {
  ...
  const compactImages=await evaluate(ui,"(async()=>{const {createPdf}=await import('../pdf.js');const {openPdf}=await import('../assets.js');const canvas=document.createElement('canvas');canvas.width=400;canvas.height=120;const ctx=canvas.getContext('2d');ctx.fillStyle='#356b50';ctx.fillRect(0,0,400,120);const data=canvas.toDataURL('image/png');const c={title:'Compact attachment flow',messages:[{role:'user',blocks:[{type:'paragraph',text:'Before images marker'},{type:'asset',assetId:'a',name:'First image'},{type:'asset',assetId:'b',name:'Second image'},{type:'paragraph',text:'After images marker'}]}],assets:['a','b'].map(id=>({id,name:'Image '+id,kind:'image',status:'ready',data,width:400,height:120}))};const blob=await createPdf(c,'light');const t=await openPdf(await blob.arrayBuffer());const p=await t.promise;const page=await p.getPage(1);const text=(await page.getTextContent()).items.map(x=>x.str).join(' ');const viewport=page.getViewport({scale:1.3});canvas.width=viewport.width;canvas.height=viewport.height;await page.render({canvasContext:canvas.getContext('2d'),viewport,intent:'print'}).promise;const png=canvas.toDataURL('image/png').split(',')[1];const count=p.numPages;await t.destroy();return {count,text,png}})()");
  assert.equal(compactImages.count,1,'Two landscape image cards and surrounding query text should share one page.');
  assert(compactImages.text.includes('Before images marker')&&compactImages.text.includes('After images marker'));
  await writeFile(join(output,'pdf-compact-images.png'),Buffer.from(compactImages.png,'base64'));
  console.log('Two bordered image cards and surrounding query text share one PDF page.');
