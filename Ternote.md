...
The user approved the UI, PDF identity and compact attachment-layout checkpoint on 2026-09-17, and confirmed native PPTX original downloading in ChatGPT on 2026-09-18. The next candidate adds PPTX reflow and attachment-type selection. Recognized ChatGPT PDF/DOCX/PPTX and common text-file cards use visible viewer download controls. PDF/DOCX resources use the existing conversion pipeline; PPTX slide text, tables and embedded raster images are reflowed within the attachment's conversation position. Originals remain available unchanged. No demo or sample UI is included. See [next steps](docs/NEXT-STEPS.md) for the ordered feature backlog and restart handoff.
...
PPTX reflow uses an independently implemented package reader and the browser's DecompressionStream in a cancellable worker, with a 60-second unpacking timeout. Actual decompressed lengths and CRC32 checksums are checked; selected parts total at most 128 MiB, with 8 MiB per XML part and 32 MiB per image part. Rendered presentation images are limited to 128 MiB. Slide order follows the presentation relationship list, not filenames. No external relationships are fetched. Reflow does not reproduce layouts, master-slide content, notes, shapes, charts, SmartArt, audio/video or animations; explicit notes retain that limitation. See [Microsoft's slide structure reference](https://learn.microsoft.com/en-us/office/open-xml/presentation/working-with-presentation-slides).
